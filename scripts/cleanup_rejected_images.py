#!/usr/bin/env python3
"""
Clean up rejected images from Milvus database and filesystem.
- Finds all records where KallisteAssessment == "reject"
- Moves image files to images_delete folder
- Removes records from Milvus database

Usage:
    python cleanup_rejected_images.py --dry-run  # See what would be deleted
    python cleanup_rejected_images.py            # Actually delete
"""
import sys
import os
...
def find_rejected_images(db: MilvusDB) -> List[Dict[str, Any]]:
    """Find all images marked as rejected in Milvus."""
    
    filter_expr = 'kalliste_assessment == "reject"'
    
    output_fields = [
        "id", "image_file_path", "kalliste_assessment", 
        "nima_assessment_technical", "nima_assessment_overall",
        "nima_score_technical", "nima_score_aesthetic",
        "person_name", "source_type"
    ]
    
    logger.info("Querying for rejected images...")
    ...
    try:
        ...
        logger.info(f"Found {len(all_results)} rejected images total")
        return all_results
...
def print_summary_stats(rejected_images: List[Dict[str, Any]]):
    ...
    logger.info(f"\n{'='*50}")
    logger.info("REJECTED IMAGES SUMMARY")
    logger.info(f"{'='*50}")
...
def main():
    parser = argparse.ArgumentParser(description="Clean up rejected images from Milvus and filesystem")
    ...
    logger.info(f"=== CLEANING UP REJECTED IMAGES ===")
    ...
    try:
        ...
        logger.info("\n" + "="*50)
        logger.info("STEP 1: Finding rejected images")
        logger.info("="*50)
        
        rejected_images = find_rejected_images(db)
