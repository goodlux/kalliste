...
def get_all_photos_for_person(db: MilvusDB, person_name: str) -> List[Dict[str, Any]]:
    ...
    results = db.query(filter_expr=filter_expr, output_fields=output_fields)

    logger.info(f"Found {len(results)} photographs for {person_name}")
    return results


def get_all_videos_for_person(
    db: MilvusDB, person_name: str, min_technical_score: float = 0.45
) -> List[Dict[str, Any]]:
...
def get_best_video_stills_for_person(
    db: MilvusDB, person_name: str, count: int, min_technical_score: float = 0.45
) -> List[Dict[str, Any]]:
    ...
    logger.info(
        f"Using clustering-first approach for {person_name} (min technical score: {min_technical_score})"
    )
...
def get_all_videos_with_iterator(
    db: MilvusDB, filter_expr: str
) -> List[Dict[str, Any]]:
    ...
    iterator = collection.query_iterator(
        batch_size=batch_size, expr=filter_expr, output_fields=output_fields
    )
    ...
    try:
        while True:
            ...
            logger.info(
                f"Batch {batch_count}: {len(batch_results)} records, {batch_with_embeddings} with embeddings"
            )

            # Progress update every 10 batches
            if batch_count % 10 == 0:
                logger.info(
                    f"Progress: {len(all_results)} total videos collected so far..."
                )
    ...
    logger.info(
        f"Completed: collected {len(all_results)} videos with embeddings in {batch_count} batches"
    )
    return all_results


def cluster_and_select_diverse_videos(
    video_results: List[Dict[str, Any]], target_count: int
) -> List[Dict[str, Any]]:
    ...
    for cluster_id in range(target_count):
        ...
        cluster_videos.sort(
            key=lambda x: x.get("nima_score_technical", 0.0), reverse=True
        )
        ...
        cluster_stats.append(
            {
                ...
                "avg_technical": sum(
                    v.get("nima_score_technical", 0.0) for v in cluster_videos
                )
                / len(cluster_videos),
            }
        )
    ...
    logger.info(
        f"Successfully selected {len(selected_videos)} diverse videos from {target_count} clusters"
    )
    ...
    if cluster_stats:
        ...
        logger.info(
            f"Selected technical scores: {min(tech_scores):.3f} - {max(tech_scores):.3f}, avg={sum(tech_scores) / len(tech_scores):.3f}"
        )
...
def create_dataset_summary(
    ...
    with open(summary_path, "w") as f:
        ...
        if clustering_used:
            f.write(
                f"Selection method: Clustering-First (ALL videos clustered for maximum diversity)\n"
            )
        else:
        ...
        if photo_results:
            photo_nima_scores = [
                r.get("nima_score_technical", 0) for r in photo_results
            ]
            f.write(
                f"  NIMA Technical Score Range: {min(photo_nima_scores):.3f} - {max(photo_nima_scores):.3f}\n"
            )
            f.write(
                f"  Average NIMA Technical: {sum(photo_nima_scores) / len(photo_nima_scores):.3f}\n"
            )
        f.write(
            f"  Files copied: {images_stats['photos_copied']} images, {images_stats['photo_sidecars']} sidecars\n\n"
        )

        # Video summary
        if clustering_used:
            f.write(
                f"VIDEO STILLS: {len(video_results)} images (clustered ALL videos into {len(video_results)} diverse groups, best quality from each)\n"
            )
        else:
            f.write(
                f"VIDEO STILLS: {len(video_results)} images (ALL videos meeting quality threshold)\n"
            )

        if video_results:
            video_nima_scores = [
                r.get("nima_score_technical", 0) for r in video_results
            ]
            f.write(
                f"  NIMA Technical Score Range: {min(video_nima_scores):.3f} - {max(video_nima_scores):.3f}\n"
            )
            f.write(
                f"  Average NIMA Technical: {sum(video_nima_scores) / len(video_nima_scores):.3f}\n"
            )
        f.write(
            f"  Files copied: {images_stats['videos_copied']} images, {images_stats['video_sidecars']} sidecars\n\n"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Create character training dataset with automatic remote sync"
    )
    parser.add_argument(
        "--person", required=True, help="Person identifier (e.g., KyCoLux)"
    )
    parser.add_argument(
        "--video-count",
        type=int,
    ...
    parser.add_argument(
        "--output-base",
        default="/Volumes/m01/kalliste_data/datasets",
    ...
    try:
        ...
        if args.video_count:
            ...
            video_results = get_all_videos_for_person(
                db, args.person, min_technical_score=args.min_technical
            )
        ...
        logger.info(
            f"Total: {photos_copied + videos_copied} images, {photo_sidecars + video_sidecars} sidecars"
        )
        ...
        if not args.dry_run:
            ...
            if not args.no_sync:
                logger.info(
                    f"Remote location: lux@clxbox:/mnt/sda2/lora_source/{args.person}"
                )
        else:
